...
import spring.config.auth.dto.OAuthAttributes;
import spring.config.auth.dto.SessionUser;
import spring.domain.user.User;
import spring.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String registrationId = userRequest.getClientRegistration().getRegistrationId(); // 로그인 진행 중인 서비스를 구분하는 코드 -> 구글, 페이스북, 네이버 ... 등 구분
